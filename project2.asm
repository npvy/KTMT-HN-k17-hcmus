...
.text 
	...
	#sao chep nam nhap vao de 2 bien nam cong tru dan xen nhau.
	addi $t0, $v0,0  
	addi $t1, $v0,0  
	
	jal Month
	addi $t2, $v0,0    #t2: Month

	jal Day
	addi $t3, $v0,0    #t3: Day

        
        add $t4,$0,0      # t4: dem so nam nhuan gan    
        
InNotLeap: #khi nam khong nhuan
        add $a0,$a2,$0      #gan nam vao $a0 de dung ham kiem tra nam nhuan.
        jal isLeapYear
	bne $v0,$0,InIsLeap #neu la nam nhuan se nhay vao nhan InIsLeap
	j SubYear           #tien hanh tang giam xen ke, vi la nam khong nhuan nen tang truoc hay giam truoc deu duoc.

SubYear:
	addi $t0,$t0,-1         #year--
	addi $a3,$t0,0		#sao chep nam vua thay doi gia tri. de khi vao ham exit se khong phan biet $t0,$t1
	
	add $a0,$t0,$0		#gan nam vao $a0 de dung ham kiem tra nam nhuan.
	jal isLeapYear
	beq $v0,$0,AddYear	#neu khong phai la nam nhuan thi se nhay ve nhan AddYear de tien hanh them giam nam theo hinh thuc xen ke.
	jal NearLeapYear  	#khi la nam nhuan thi se nhay sang nhan NearLeapYear
AddYear:
	addi $t1,$t1,1          #year++
        add $a3,$t1,$0 		#sao chep nam vua thay doi gia tri. de khi vao ham exit se khong phan biet $t0,$t1
        
        add $a0,$t1,0	        #gan nam vao $a0 de dung ham kiem tra nam nhuan.
	jal isLeapYear
	beq $v0,$0,SubYear	#neu khong phai la nam nhuan thi se nhay ve nhan SudYear de tien hanh giam nam theo hinh thuc xen ke.
	jal NearLeapYear	#khi la nam nhuan thi se nhay sang nhan NearLeapYear
	
InIsLeap: #xu li truong hop nam nhap vao la nam nhuan.
	#ngay 2/7 trong nam nhuan duoc xem la ngay phan tach 2 phan cua nam. neu ngay sau 2/7 thi se gan voi nam X+1. 
	#neu ngay nhap truoc hoac bang 2/7 thi se gan voi X-1 ( X la nam nhap vao)
	addi $k0,$0,7 #thang 7
	addi $k1,$0,2 #ngay 2
	
	beq $t2,7,BeqMonth     #neu thang nhap vao la thang 7 thi nhay vao nhan BeqMonth de xu li
	
	slt $t5,$t2,$k0        #thang nhap vao < 7 thi tien hanh tru truoc cong sau.
	beq $t5,1,SubYear
	
	sgt $t5,$t2,$k0        # thang nhap vao > 7 thi tien hanh cong truoc tru sau.
	beq $t5,1,AddYear

BeqMonth:
	sle $t5,$t3,$k1        # ngay nhap vao <=2 thi tien hanh tru truoc cong sau.
	beq $t5,1,SubYear
	j AddYear              # ngay nhap vao > 2 thi tien hanh cong truoc tru sau.

NearLeapYear:
	addi $t4,$t4,1         #$t4 de dem so nam nhuan gan da tim duoc. 
	beq $t4,2,exitTNLY     #khi t4=2 co nghia da du so nam can tim thi tien hanh nhay toi nhan  exitTNLY 
	add $a1,$a3,$0         # khi s5=1 luu ket  qua vao $t4
	slt $t5,$a3,$a2        #kiem tra nam nhuan gan nhat vua tim duoc lon hay nho hon nam nhap vao.
	beq $t5,1,AddYear      # neu nho hon thi bat dau cong truoc
	j SubYear              # lon hon bat dau tru truoc.

exitTNLY:
        add $v0,$a1,$0	       # nam nhuan gan thu nhat. 
	add $v1,$a3,$0  # nam nhuan gan thu hai.
